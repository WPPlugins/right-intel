...
if ($numConnected > 0) {
	$msgs[] = 'Connected ' . $numConnected .' Right Intel instance' . ($numConnected == 1 ? '' : 's') . '.';
}
if ($numDisconnected > 0) {
	$msgs[] = 'Disconnected ' . $numDisconnected .' Right Intel instance' . ($numDisconnected == 1 ? '' : 's') . '.';
}
...
$hasConnectedBefore = get_option('right_intel_has_connected_before');
