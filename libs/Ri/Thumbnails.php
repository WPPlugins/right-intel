...
class Ri_Thumbnails {
	...
	public static function upgradeIfNeeded(Ri_Router $router) {
		...
		if ( get_option( 'siteurl' ) . $_SERVER['REQUEST_URI'] != $pageUrl ) {			
			$router->addAdminNotice('Thank you for updating the Right Intel WordPress plugin. Please <a href="' . esc_html($pageUrl) . '">click here</a> to complete the upgrade process.');
		}
	}
	...
	*/
	...
	public static function convert($post_id) {
		...
		if (!isset($meta['ri_post_id'])) {
			// not a right intel post   || already processed
			return false;
		}
