...
/**
 * Handle API authentication between WordPress server and Right Intel server
...
class Ri_Credentials {
	 ...
	 * The name of the instance as seen in Right Intel
	 * @var string
	 */
	...
	public static function getConnectionUrl() {
		$host = isset($_SERVER['HTTP_HOST']) ? strtolower($_SERVER['HTTP_HOST']) : 'rightintel.com';
		if ($host == 'rightintel' || $host == 'kds') {
			return 'http://rightintel/client_blogs/choose_instance';
		}
		if ($host == 'dev.rightintel.com') {
			return 'https://dev.rightintel.com/client_blogs/choose_instance';
		}
		return 'https://rightintel.com/client_blogs/choose_instance';
	}
	...
	public function verifyConnection($wp_user_id, $timestamp, $hash) {
		...
		if ($expectedHash == $hash) {
			// TODO: check if timestamp is within 30 minutes
			return true;
		}
