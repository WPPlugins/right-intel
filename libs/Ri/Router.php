...
class Ri_Router {
	...
	public static function validateInstall() {
		if (trim(get_option('permalink_structure')) == '') {
			Ri_Flash::add("<strong>Error:</strong> WordPress permalinks must be enabled. Go to Settings &rsaquo; Permalinks and choose something other than the default.", 'error');
			return false;
		}
		return true;
	}
	...
	public function addPage($info) {
		...
		$pageUrl = get_option('home') . '/wp-admin/' . $info['parent_slug'] . '?page=' . $info['menu_slug'];
	...
	/**
	 * Add a link to right intel settings page on the plugin page
