...
class Ri_Styling {
	...
	public function setupCss() {
		...
		$this->addCss( 'init', static::getCssRelativeUrl() );
	 ...
	 * @return \Ri_Router
	 */
	public function addCss($onAction, $file) {
		$url = plugins_url("/css/$file", RI_BASE_PAGE);
		$name = md5($file);
		add_action($onAction, function() use($name, $url) {
			wp_enqueue_style( $name, $url, array(), RI_VERSION . '-' . get_option('right_intel_max_post_id'), 'all' );
		});
		return $this;
	}
